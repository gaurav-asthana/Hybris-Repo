 ...
 */
package de.hybris.kickerecommerce.request.mapping.handler;
...
{
	private static final Logger logger = LoggerFactory.getLogger(CommerceHandlerMapping.class);
	...
	protected void initOverridenRequestMappings()
	{
		...
		{
			logger.debug("Looking for overridden request mappings in application context: " + getApplicationContext());
		}
	...
	protected void registerOverriddenMapping(final Method method, final RequestMappingInfo mapping)
	{
		...
		if (newPriority != null)
		{
			...
			{
				logger.info("Mapping \"" + mapping + "\" overridden with priority = " + newPriority);
			}
		}
	}
